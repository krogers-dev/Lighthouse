...
test(
  'NEGATIVE: missing configuration stops the lane BEFORE any export runs',
  { skip: POSIX_ONLY },
  () => {
...
test('NEGATIVE: a failing export fails the lane', { skip: POSIX_ONLY }, () => {
...
test(
  'NEGATIVE: an export that writes NOTHING fails — stale/preplanted output elsewhere can never stand in',
  { skip: POSIX_ONLY },
  () => {
...
test('NEGATIVE: sub-floor payloads fail platform verification', { skip: POSIX_ONLY }, () => {
  const tinyBody = [
    'mkdir -p "$OUT/_expo/static/js/ios" "$OUT/_expo/static/js/android" "$OUT/_expo/static/js/web"',
...
test(
  'NEGATIVE: a missing platform in metadata fails without hardcoded counts',
  { skip: POSIX_ONLY },
  () => {
...
test(
  'a clean export passes; the run record binds command, tools, commit, config digest, and output manifest',
  { skip: POSIX_ONLY },
  () => {
...
test(
  "NEGATIVE: a planted QA marker in THIS run's output fails the lane (inspection reads the fresh dir)",
  { skip: POSIX_ONLY },
  () => {
    const planted = GOOD_BODY.replace(
      'if [ "$EXPO_PUBLIC_QA_HOOKS" = "1" ]; then',
      'if true; then',
    );
...
test(
  'the QA-enabled positive control still exists and PASSES only when inspection rejects the export',
  { skip: POSIX_ONLY },
  () => {
    const { pathDir } = makeFakeNpx(GOOD_BODY);
