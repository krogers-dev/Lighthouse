...
| `SCOPE_SWITCH_REQUESTED`               | authorized                                                | select_scope                                  | Controller re-reads memberships from the server first (find 38), then clears all scoped repositories                                                                                                             |
| `MEMBERSHIPS_REFRESHED {memberships}`  | authorized, select_scope                                  | same state, memberships replaced              | Server truth after sign-in (find 38); the bound scope is kept even when its membership is gone, so read surfaces render stale scope; an empty set is illegal here — the controller signs out (no_access) instead |
| `SIGN_OUT_REQUESTED {reason}`          | authorized, select_scope, mfa_required, first_factor      | signing_out                                   | Also used for identity switch, expiry, and no_access (every membership revoked underneath a live session)                                                                                                        |
