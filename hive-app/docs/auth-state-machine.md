...
| `mfa_required` | First factor passed; TOTP challenge outstanding or verifying | `verifying`, `notice?` |
...
| `MFA_SUBMITTED` | mfa_required | mfa_required(verifying) | |
| `MFA_FAILED {code}` | mfa_required(verifying) | mfa_required(notice) | |
...
| `RETURN_TO_SIGNED_OUT` | first_factor | signed_out | Safe back/cancel before verification |
...
controller starts a TOTP challenge → `MFA_CHALLENGE_REQUIRED`; MFA verify →
memberships → `SCOPES_LOADED`.
